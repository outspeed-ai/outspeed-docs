...
<Steps>
  <Step title="Run Backend">
    <Warning>
    Ensure that you've followed the steps from [QuickStart](/examples/quickstart).
    Navigate to the base directory of the cloned outspeed repository.
    </Warning>
    ...
    ```bash
    cd examples/sports_commentator/poker_commentator.py
  ...
  4. Select your Audio device. Select the resolution (works better with higher resolution). Click Run to begin.
...
- **CartesiaTTS**: Converts text responses back into spoken audio using Eleven Labs' text-to-speech API, optimized for low latency and using a specific voice model.
