...
import InstallOutspeedSnippet from "/snippets/install-outspeed.mdx";
  ...
  <Step title="Run Backend">
    <Warning>
      Ensure you’re in the same directory as voice_bot_with_online_search.py!
    </Warning>
    ...
    <Tab title="Local Server">
      ...
      2. `EXA_API_KEY` - You can get this by visiting the [Exa API documentation](https://exa.ai/api) and navigating to the API keys section to generate a new key.
    ...
    <Tab title="Outspeed Cloud">
      ...
      ```bash
      outspeed deploy --api-key=<your-api-key> voice_bot_with_online_search.py
      ```
...
Review the code in `voice_bot_with_online_search.py`.
