...
You can deploy your app using the `outspeed deploy` CLI command and an API key.
To obtain an API key, please visit our website at https://www.outspeed.com and sign up.
...
<Step title="Setup environment and install dependencies.">
    <CodeGroup>
    ```bash MacOS/Linux
    # Create a virtual environment. Make sure python version >=3.9, <3.12
    python3 -m venv .venv
    ...
    ```batch Windows
    REM Create a virtual environment. Make sure python version >=3.9, <3.12
    python -m venv .venv
