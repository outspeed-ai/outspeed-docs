...
description: "Receive notifications when voice sessions complete"
---

## Overview

Webhooks allow you to receive notifications when voice sessions complete. When a session ends, Outspeed automatically sends session data (including the full conversation transcript) to your specified webhook endpoints via HTTP POST requests.
...
def verify_webhook(raw_body: bytes, signature_header: str, webhook_secret: str) -> bool:
    ...
    Args:
        ...
        signature_header: Value from X-Outspeed-Signature header
        webhook_secret: Webhook secret from Outspeed dashboard
...
is_valid = verify_webhook(raw_body, signature, 'whsec_your_secret_here')
```
...
| `timestamp`  | integer | Unix/POSIX timestamp (seconds since epoch) when webhook was sent     |
| `user_id`    | string  | User who created the session                                         |
| `context`    | array   | Full conversation transcript with role, content, and Unix timestamps |

## Replay Attack Prevention

The `timestamp` field is a Unix/POSIX timestamp (seconds since epoch) that allows you to reject old webhook requests:

```python
import time

def is_recent(webhook_timestamp, tolerance_seconds=600):
    """Reject webhooks older than 10 minutes"""
    current_time = int(time.time())
    age = current_time - webhook_timestamp
    return age <= tolerance_seconds
```

**Recommended tolerance**: 600 seconds (10 minutes)
...
<Warning>Deleting a webhook is permanent. You'll need to create a new webhook and receive a new secret.</Warning>
...
Add comprehensive logging to help debug webhook issues:
...
async def webhook(request: Request):
    ...
    if signature:
        ...
        logger.warning("Missing X-Outspeed-Signature header")
    ...
    logger.info(f"Messages: {len(payload.get('context', []))}")
...
- **Request received**: Log timestamp and basic request info
- **Signature verification**: Log success/failure (not the actual secret)
- **Timestamp validation**: Log webhook age
- **Payload processing**: Log session ID and key fields
- **Errors**: Always log the error type and context
...
5. **Check webhook secret** - Ensure `WEBHOOK_SECRET` environment variable is set

### Signature Verification Fails

- Ensure you're using the correct secret (from creation time)
- **CRITICAL**: Use the raw request body for verification, not the parsed JSON
...
- Confirm the signature header is being read correctly (case-sensitive)

### Missing Signature Header

If you receive "Missing signature header" errors:

- Check header name is exactly `X-Outspeed-Signature` (case-sensitive)
- Verify your framework isn't normalizing header names
- Confirm the request is actually from Outspeed (check delivery logs)
...
- Add timeout monitoring to your logs
...
- Increase your server's max request body size if needed
