...
def verify_webhook(raw_body: Union[str, bytes], signature_header: str, webhook_secret: str) -> bool:
...
# Usage (example with any FastAPI)
signature = request.headers.get('X-Outspeed-Signature')
raw_body = await request.body()  # Get raw body bytes
...
// Ensure express js is parsing the RAW body
app.use(bodyParser.raw({ type: '*/*' }));

app.post("/webhook", (req, res) => {
  const signature = req.headers["x-outspeed-signature"];
  const rawBody = req.body; // Raw body is now available via req.body
  const payload = JSON.parse(rawBody.toString()); // Parse the raw body to get the payload object
...
async def webhook(request: Request):
    signature = request.headers.get("X-Outspeed-Signature")
    raw_payload = await request.body()  # Get raw body as bytes

    # 1. Verify signature using raw bytes
    expected_sig = 'sha256=' + hmac.new(
        WEBHOOK_SECRET.encode(),
        raw_payload,  # raw_payload is already bytes, use directly
        hashlib.sha256
    ).hexdigest()
