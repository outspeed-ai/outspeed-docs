...
const sessionConfig: SessionConfig = {
  ...
  // input_language: "zh", // Allow users to speak in Chinese (optional)
};
  ...
  const startSession = async () => {
    try {
      ...
      await conversation.startSession(ephemeralKey, {
        source: "quickstart-app" // Optional: custom string to identify session source
      });
    } catch (error) {
...
<Note>Need users to speak in a different language? Configure [input language](/features/input-language) to support English, Hindi, or Chinese.</Note>
